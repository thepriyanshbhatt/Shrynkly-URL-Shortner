	...
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		if (url.pathname === '/api/shorten' && request.method === 'POST') {
			try {
				...
				// Store in KV with a 3-day expiration (259200 seconds) by default for free tier
				await env.LINKS.put(shortCode as string, parsedUrl.toString(), { expirationTtl: 259200 });
